...
pub async fn convert_with_emoji_overlays(
    ...
    let _ = app.emit("export_log", "🎨 Using IMAGE-BASED emoji system (guaranteed to work!)");
    ...
    if Path::new(emoji_dir).exists() {
        ...
        let _ = app.emit("export_log", "📦 Generating emoji images (first time only)...");
    ...
    let filter_output = Command::new("python3")
        .args(["generate_emoji_overlays.py", subtitle_file])
        .output()
        .await
